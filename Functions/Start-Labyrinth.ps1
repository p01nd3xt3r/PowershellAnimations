...
Function Start-Labyrinth {
    ...
    If ($Type -eq "box") {
        ...
        # Navigating directories by a simple up/down marker and hitting left or right to go to them. Hitting enter on a file opens it with the default program. Maybe make a way to open with a different program.
        # Do you set location to the directory or just write contents and then just give them the option to set their location? What other options do you give them?
        # Right arrow digs into directory, left arrow goes back one.
        # Keep history so that you can keep going back to where you started.
        
        $LabUserCharacter = "●"
        $LabDirPathHistoryHash = [ordered]@{}
        
        Function ArchiveLocation {
            param (
                [Parameter()][String]$Path = (Get-Location)
            )
            #Adding current path to history
            $LabDirPathHistoryI = $LabDirPathHistoryHash.count + 1
            $LabDirPathHistoryHash.add($LabDirPathHistoryI,$Path)
            Return $Path
        }
        ...
        $LabDirPath = ArchiveLocation
        $LabDirHash = BuildLocationHash -path $LabDirPath
        BuildOutput
